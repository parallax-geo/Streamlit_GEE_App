...
try:
    def get_era_temp_image(selected_sub_basin, from_date, to_date):
        ...
        temp_collection = ee.ImageCollection("ECMWF/ERA5_LAND/DAILY_AGGR").filterBounds(sub_basin_feature).filterDate(start_date, end_date).select('temperature_2m').map(scale_index)
